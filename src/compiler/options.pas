    ...
    '-' :
      begin
         ...
         case opt[2] of
           ...
           'A' :
             begin
               if (length(More)>=2) and (More[1]='o') then
                 begin
                   { -Ao passes extra options to the external assembler }
                   asmextraopt:=copy(More,2,length(More)-1);
                 end
               else
                 begin
...
{$endif defined(arm) or defined(avr)}
               ...
               { -WS<file> : custom go32v2 DOS stub (e.g. cwsdstub.exe for embedded DPMI) }
               if (length(More)>=2) and (More[1]='S') then
                 begin
                   go32v2stubfile:=copy(More,2,length(More)-1);
                 end;
             end;
