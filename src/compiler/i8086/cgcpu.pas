  ...
  implementation
    ...
    procedure tcg8086.init_register_allocators;
      begin
        ...
        { i8086 has no MMX/XMM registers - removed for backport }
        { rg[R_MMREGISTER] - not available on i8086 }
        ...
        case tosize of
          ...
          OS_16,OS_S16:
            case fromsize of
              ...
              OS_32,OS_S32:
                { i8086 backport: truncating load of a 32-bit source into a
                  16-bit register — just read the low word (the reference's
                  first 16 bits); the high word is discarded. }
                list.concat(taicpu.op_ref_reg(A_MOV, S_W, tmpref, reg));
              else
            ...
            case tosize of
              ...
              OS_16,OS_S16:
                case fromsize of
                  ...
                  OS_32,OS_S32:
                    { i8086 backport: truncating a 32-bit register pair into a
                      16-bit register — just move the low half (reg1); the high
                      half (GetNextReg(reg1)) is discarded. }
                    begin
                      if reg1<>reg2 then
                        add_mov(taicpu.op_reg_reg(A_MOV, S_W, reg1, reg2));
                    end
          ...
          begin
            if (current_procinfo.framepointer=NR_STACK_POINTER_REG) then
              begin
                ...
                if (2 {i8086 stackalign}>4) and
                   ((stacksize <> 0) or
                    (pi_do_call in current_procinfo.flags) or
                    { can't detect if a call in this case -> use nostackframe }
                    { if you (think you) know what you are doing              }
                    (po_assembler in current_procinfo.procdef.procoptions)) then
                  stacksize := align(stacksize+sizeof(aint),2 {i8086 stackalign}) - sizeof(aint);
                if (stacksize<>0) then
...
{$endif volatile_es}
        ...
        { i8086 backport: 2.6.4 has no targetswitches/ts_cld subsystem. Always
          emit CLD before string ops (safe default: ensures forward direction). }
