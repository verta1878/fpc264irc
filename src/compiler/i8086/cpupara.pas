...
****************************************************************************}
    ...
    function ti8086paramanager.ret_in_param(def:tdef;calloption:tproccalloption):boolean;
      begin
        { i8086 backport: adapted to 2.6.4's calloption-based signature.
          i8086-specific rule: aggregates larger than 8 bytes (and not
          floats) are returned via an implicit pointer parameter. }
    ...
    procedure ti8086paramanager.getintparaloc(calloption : tproccalloption; nr : longint; var cgpara : tcgpara);
      var
        paraloc : pcgparalocation;
      begin
        cgpara.reset;
    ...
    procedure ti8086paramanager.create_funcretloc_info(p : tabstractprocdef; side: tcallercallee);
      begin
        p.funcretloc[side]:=get_funcretloc(p,side,p.returndef);
      end;


    function  ti8086paramanager.get_funcretloc(p : tabstractprocdef; side: tcallercallee; def: tdef): TCGPara;
      var
        retcgsize  : tcgsize;
        paraloc : pcgparalocation;
      begin
        { i8086 backport: rewritten in 2.6.4 style (result.init, no
          set_common_funcretloc_info / cgpara.def / .temporary), preserving the
          i8086 return convention: 64-bit results split across 4 x 16-bit regs,
          32-bit across 2 x 16-bit regs. }
        result.init;
          ...
          begin
            ...
            if pushaddr then
              begin
                ...
                paradef:=tpointerdef.create(paradef);
              end
            else
              begin
            ...
            if (paracgsize=OS_NO) or
               (use_fixed_stack) or
               { i8086 backport: floating-point values on the stack must be a
                 single contiguous LOC_REFERENCE — the FPU loads them as one
                 value (a_loadfpu_reg_cgpara calls check_simple_location). If
                 they were split into per-16-bit locations (the else branch
                 below), that check fails (internalerror 200408162). }
                      ...
                      if pass=2 then
                        begin
                          ...
                          if (use_fixed_stack) or
                             (paracgsize=OS_NO) or
                             { i8086 backport: a float on the stack must be a
                               single contiguous LOC_REFERENCE (the FPU loads it
                               as one value; a_loadfpu_reg_cgpara calls
                               check_simple_location). Splitting into per-16-bit
                               locations breaks that (internalerror 200408162). }
                             (paradef.typ=floatdef) then
                            begin
                              paraloc:=hp.paraloc[side].add_location;
                              paraloc^.loc:=LOC_REFERENCE;
