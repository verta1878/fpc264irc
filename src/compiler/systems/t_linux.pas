...
begin
  with Info do
   begin
     ...
     { r3 fix (ld linker-invocation bug): link.res is a full replacement linker
       script (ENTRY + SECTIONS), so it must be passed with -T. Passing it as a
       bare positional argument makes modern GNU ld (binutils 2.19+, e.g. 2.42)
       treat it as an ambiguous input file — it emits "link.res contains output
       sections; did you forget -T?" and on some binutils builds parses it
       pathologically slowly or wedges the ld process entirely. FPC 3.2.2 fixed
       this by always using -T for binutils >= 2.19; we always emit a full
       script, so -T is always correct here. (Cross-links with a sysroot also
       need it, which the old code already handled.) }
     ExeCmd[1]:=ExeCmd[1]+' -T';
