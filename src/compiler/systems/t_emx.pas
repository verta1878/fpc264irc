...
begin
  with Info do
   begin
     { -Ttext 0x10000 places the text segment at EMX's TEXT_BASE so the
       a.out entry point equals TEXT_BASE, which emxbind requires when
       binding the executable. }
     ExeCmd[1]:='ld $OPT -N -Ttext 0x10000 -o $OUT @$RES';  { -N = OMAGIC for emxbind }
