...
Type
  ...
  Public
    ...
    Function  LoadJPG       (FileName: String; X, Y: SmallInt) : Boolean;  // JPEG (stub)
    Function  LoadRFF       (AFontNum: Byte; FileName: String) : Boolean;  // RFF font (stub)
...
// Pixel decoding requires pasjpeg package (58 units) — not linked
...
// Maps to CHR font slot — RFF glyphs rendered as vector strokes
Var
  F         : File;
...
Begin
  ...
  // Parse header
  DataSize  := Hdr[0] OR (Hdr[1] SHL 8);
  // Descriptor at offset 0x10
  // Version check: major=Hdr[0x12], minor=Hdr[0x13]
  If Hdr[$12] <> 2 Then Begin Close(F); Exit; End;  // only v2.x
  ...
  // Allocate CHR font slot for this RFF
  ...
  With CHRFonts[AFontNum]^ Do Begin
    ...
    FirstChar := FirstChar;
    OrgToCap  := Ascent DIV 10;
    If OrgToCap = 0 Then OrgToCap := 14;
    OrgToBase := 0;
    OrgToDec  := 0;

    For I := 0 to NumChars - 1 Do Begin
      Widths[I] := MaxWidth DIV 10;
      If Widths[I] = 0 Then Widths[I] := 8;
      Offsets[I] := 0;
    End;

    // Note: full glyph stroke data decoding not yet implemented.
    // The font slot is registered so |f can reference it and
    // text rendering falls back to the bitmap system font.
    NumStrokes := 0;
  End;
...
Begin
  ...
  For Y := 0 to DstH - 1 Do Begin
    ...
    For X := 0 to DstW - 1 Do Begin
      ...
      PByte(LongInt(DstP) + Y * DstW + X)^ :=
        PByte(LongInt(SrcP) + SY * SrcW + SX)^;
    End;
  End;
End;
  ...
  For Y := 0 to H - 1 Do
    For X := 0 to W - 1 Do Begin
      ...
      If (SX >= 0) and (SX < W) and (SY >= 0) and (SY < H) Then
        PByte(LongInt(DstP) + Y * W + X)^ :=
          PByte(LongInt(SrcP) + SY * W + SX)^
      Else
        PByte(LongInt(DstP) + Y * W + X)^ := BackFill;
    End;
End;
  ...
  For IY := 0 to H - 1 Do
    For IX := 0 to W - 1 Do
      If InView(X + IX, Y + IY) Then Begin
        PByte(LongInt(SprP) + IY * W + IX)^ := Pixels^[Y + IY, X + IX];
        PByte(LongInt(BkgP) + IY * W + IX)^ := Pixels^[Y + IY, X + IX];
      End;
End;
...
Begin
  ...
  W := PByte(SprP)^;
  H := PByte(LongInt(SprP) + 1)^;
  If (W = 0) or (H = 0) Then Exit;
  For IY := 0 to H - 1 Do
    For IX := 0 to W - 1 Do Begin
      C := PByte(LongInt(SprP) + 2 + IY * W + IX)^;
...
Begin
  Result := False;
  P1 := @S1; P2 := @S2;
  W1 := PByte(P1)^; H1 := PByte(LongInt(P1) + 1)^;
  W2 := PByte(P2)^; H2 := PByte(LongInt(P2) + 1)^;
  ...
  For IY := OY0 to OY1 Do
    For IX := OX0 to OX1 Do
      If (PByte(LongInt(P1) + 2 + (IY-Y1)*W1 + (IX-X1))^ <> TransColor) and
         (PByte(LongInt(P2) + 2 + (IY-Y2)*W2 + (IX-X2))^ <> TransColor) Then Begin
        Result := True;
        Exit;
      End;
End;
...
Begin
  ...
  Case Cmd of
    ...
    'f' : Begin
            ...
            X0 := MegaNum(Params, P, 2);
            Y0 := MegaNum(Params, P, 2);
            // RFF font loading not yet implemented — stub accepts the command
...
Begin
  ...
  Case Cmd of
    ...
    'b' : Begin
            ...
            // Parse what we can, stub the rest
            X0 := MegaNum(Params, P, 2);
            Y0 := MegaNum(Params, P, 2);
            X1 := MegaNum(Params, P, 2);
            Y1 := MegaNum(Params, P, 2);
            // Rest is button definition — pass through to standard handler
            // Full v2.0 button extensions TBD
          End;
