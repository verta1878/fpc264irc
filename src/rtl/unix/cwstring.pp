...
{ Unicode move procedures — wrappers around the wide versions.
  punicodechar and pwidechar are the same type on FPC, so we can
  delegate directly. The cp parameter is accepted for signature
  compatibility but cwstring uses iconv, not system codepages. }
procedure Unicode2AnsiMove(source:punicodechar;var dest:ansistring;cp:TSystemCodePage;len:SizeInt);
begin
  Wide2AnsiMove(pwidechar(source),dest,cp,len);
end;
...
var
  ws: widestring;
begin
  Ansi2WideMove(source,cp,ws,len);
  dest:=ws;
end;
