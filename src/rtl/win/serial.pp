...
begin
  ...
  Props.wPacketLength := SizeOf(Props);
  if GetCommProperties(Handle, Props) then begin
    case Props.dwProvSubType of
      $01: Result := '16450';
      $02: Result := '16550';
      $03: Result := '16550A';
      $04: Result := '16650';
...
begin
  { Windows: SetupComm sets the recommended buffer sizes.
    TriggerLevel maps to input buffer size. }
  ...
  SetupComm(Handle, InSize, OutSize);
end;
...
begin
  FillChar(Props, SizeOf(Props), 0);
  Props.wPacketLength := SizeOf(Props);
  if GetCommProperties(Handle, Props) then
    Result := Word(Props.dwProvSpec1 and $FFFF)
  else
    Result := 0;
end;
...
var
  Mask: DWORD;
begin
  { Windows: enable event notification for received data. }
  Mask := $0001; { EV_RXCHAR }
  SetCommMask(Handle, Mask);
end;

procedure SerDisableIRQ(Handle: TSerialHandle);
begin
  { Windows: disable all event notifications. }
