...
begin
  ...
  current_Map:=getmap_cp(current_DefaultSystemCodePage);
  c:=nil;
...
begin
  ...
  for i:=1 to len do
    begin
      blockLen:=getascii_buf(tunicodechar(locSource^),locMap,destBuffer,(destLen-actualLen));
      if (blockLen<0) then
        begin
...
begin
  ...
  destLen:=getunicode_buf(source,len,locMap,nil);
  SetLength(dest,destLen);
  getunicode_buf(source,len,locMap,@dest[1]);
end;
...
function CompareUnicodeString_NoOpt(const s1, s2: UnicodeString): PtrInt;
begin
  Result := CompareUnicodeString(s1, s2, []);
end;

function CompareWideString_NoOpt(const s1, s2: WideString): PtrInt;
begin
  ...
  while (i<=slen) do
    begin
      ...
      if (mblen<=0) then
        ...
        begin
          SetLength(us,2);
          ulen:=getunicode_buf(p,mblen,locMap,@us[1]);
          if (Length(us)<>ulen) then
            SetLength(us,ulen);
          usl:=UpperUnicodeString(us);
          for k:=1 to Length(usl) do
            begin
              aalen:=getascii_buf(tunicodechar(us[k]),locMap,@aa[Low(aa)],Length(aa));
...
{$ifndef FPC_WIDESTRING_EQUAL_UNICODESTRING}
      ...
      {$IFDEF CPUI8086}CompareWideStringProc:=@CompareWideString;{$ELSE}CompareWideStringProc:=@CompareWideString_NoOpt;{$ENDIF}
...
{$endif FPC_WIDESTRING_EQUAL_UNICODESTRING}
      ...
      {$IFDEF CPUI8086}CompareUnicodeStringProc:=@CompareUnicodeString;{$ELSE}CompareUnicodeStringProc:=@CompareUnicodeString_NoOpt;{$ENDIF}
    end;
...
{$ELSE MSWINDOWS}
 ...
 {$ELSE UNIX}
  { FPC 2.6.4 backport: detect codepage from OS }
  DefaultSystemCodePage := unixcp.GetSystemCodepage;
  DefaultFileSystemCodePage := DefaultSystemCodePage;
