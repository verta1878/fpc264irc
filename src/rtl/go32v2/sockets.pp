...
Var
  ...
  DMABufSel  : Word;   { protected-mode selector }
  DMABufSeg  : Word;   { real-mode segment }
  DMABufSize : Word;
...
Begin
  ...
  DMABufSize := 2048;
  AllocResult := global_dos_alloc(DMABufSize);
  DMABufSel := Word(AllocResult and $FFFF);
  DMABufSeg := Word(AllocResult shr 16);

  { Scan INT 60h-80h for packet driver signature "PKT DRVR" }
  For IntNo := PKT_INT_FIRST to PKT_INT_LAST do begin
    get_rm_interrupt(IntNo, Info);
    if Info.offset <> nil then begin
      { Read 8 bytes at ISR+3 in real-mode memory }
      seg_move(dosmemselector, LongInt(Info.segment)*16 + LongInt(PtrUInt(Info.offset)) + 3,
               get_ds, LongInt(@Sig), 8);
      ...
      begin
        ...
        Regs.ES := 0;
        Regs.DI := 0;
        realintr(PktInt, Regs);
        if (Regs.Flags and carryflag) = 0 then begin
          PktHandle := Regs.AX;
          { Get address }
          FillChar(Regs, SizeOf(Regs), 0);
          Regs.AH := $06;
          ...
          if (Regs.Flags and carryflag) = 0 then
            seg_move(dosmemselector, LongInt(DMABufSeg)*16,
                     get_ds, LongInt(@PktMAC), 6);
        end;
...
Begin
  ...
  if DMABufSel <> 0 then begin
    global_dos_free(DMABufSel);
    DMABufSel := 0;
  end;
End;
...
Begin
  ...
  seg_move(get_ds, LongInt(Data),
           dosmemselector, LongInt(DMABufSeg)*16, Len);
        ...
        tssEstablished: begin
          ...
          if DataLen > 0 then begin
            ...
            if SockTable[Sock].RecvCount + DataLen <= TCP_RXBUF then begin
              Move(PByte(PtrUInt(TCPData) + DataOfs)^,
...
Begin
  ...
  case IP^.Protocol of
    IPPROTO_TCP:
      ProcessTCPPacket(IP^, Pointer(PtrUInt(Data) + IPHdrLen), 
...
Begin
  ...
  case EthType of
    ETHER_IP:  ProcessIPPacket(Pointer(PtrUInt(Data) + 14), Len - 14);
    ETHER_ARP: ProcessARPPacket(Pointer(PtrUInt(Data) + 14), Len - 14);
  end;
End;
