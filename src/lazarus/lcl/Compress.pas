...

{
  ...
  Modified for FPC 2.6.4irc compatibility:
  - Lo(CurCRC) changed to Byte(CurCRC) in UpdateCRC32 (BUG-035)
    FPC Lo() returns Word for LongInt; Delphi returns Byte.
...
begin
  ...
  while BufSize <> 0 do begin
    CurCRC := CRC32Table[Byte(CurCRC) xor P^] xor (CurCRC shr 8);
    Dec(BufSize);
    Inc(P);
  end;
  Result := CurCRC;
