  ...
  If (Last<>Nil) Then
   Begin
     ...
     Repeat
       ...
       IF Byte(Longint(CallPointerMethodLocal(CodePointer(P),get_caller_frame(get_frame),@self,Tp)))<>0 THEN
...
BEGIN
   ...
   End Else Valid := FirstThat(Pointer(@IsInvalid)) = Nil;     { Check first valid }
END;
...
BEGIN
   ForEach(Pointer(@DoAwaken));                                { Awaken each view }
END;
  ...
  If (Last<>Nil) Then
   Begin
     ...
     Repeat
       ...
       CallPointerMethodLocal(CodePointer(P),get_caller_frame(get_frame),@self,Tp);
...
BEGIN
   ...
   Case AState Of
     sfActive, sfDragging: Begin
         Lock;                                        { Lock the view }
         ForEach(Pointer(@DoSetState));                        { Set each subview }
     ...
     sfFocused: Begin
       ...
       End;
     sfExposed: Begin
         ForEach(Pointer(@DoExpose));                          { Expose each subview }
       End;
   End;
END;
...
BEGIN
   ...
   ForEach(Pointer(@DoPut));                                   { Put each in stream }
...
BEGIN
   ...
   If (Event.What AND FocusedEvents <> 0) Then Begin  { Focused event }
     ...
     ForEach(Pointer(@DoHandleEvent));                         { Pass to each view }
     Phase := phFocused;                              { Set focused }
     DoHandleEvent(Current);                          { Pass to current }
     Phase := phPostProcess;                          { Set post process }
     ForEach(Pointer(@DoHandleEvent));                         { Pass to each }
   End Else Begin
     Phase := phFocused;                              { Set focused }
     If (Event.What AND PositionalEvents <> 0) Then   { Positional event }
       DoHandleEvent(FirstThat(Pointer(@ContainsMouse)))       { Pass to first }
       Else ForEach(Pointer(@DoHandleEvent));                  { Pass to all }
   End;
END;
...
BEGIN
   ...
   End Else Begin
     ...
     Lock;                                            { Lock drawing }
     ForEach(Pointer(@DoCalcChange));                          { Change each view }
...
BEGIN
   FirstMatch := FirstThat(Pointer(@Matches));                 { Return first match }
END;
