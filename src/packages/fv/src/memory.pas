...
BEGIN
   ...
   PSeg := Seg(P^);
   MoveSeg(PSeg + CurSize, PSeg +
     NewSize, BufHeapPtr - PSeg - CurSize);            { Move data }
   Inc(BufHeapPtr, NewSize - CurSize);                { Adjust heap space }
   PSeg := PSeg + NewSize;
   P := PBuffer(Ptr(PSeg, 0));
   While PSeg < BufHeapPtr Do Begin
     Inc(P^.Master^, NewSize - CurSize);              { Adjust master }
     PSeg := PSeg + (P^.Size + 15) SHR 4 + 1;
     P := PBuffer(Ptr(PSeg, 0));                { Adjust paragraphs }
   End;
END;
...
BEGIN
   {$IFDEF PROC_REAL}                                 { REAL MODE DOS CODE }
   {$IFNDEF CPUI8086}HeapError := @HeapNotify;{$ENDIF}                          { Point to error proc }
   If (BufHeapPtr = 0) Then Begin
     HeapSize := Seg(HeapEnd^)
       - Seg(HeapOrg^);                         { Calculate size }
     If (HeapSize > MaxHeapSize) Then
       HeapSize := MaxHeapSize;                       { Restrict max size }
     BufHeapEnd := Seg(HeapEnd^);               { Set heap end }
     Seg(HeapEnd^) := Seg(HeapOrg^)
      + HeapSize;                                     { Add heapsize }
     BufHeapPtr := Seg(HeapEnd^);               { Set heap pointer }
   End;
   CachePtr := HeapEnd;                               { Cache starts at end }
   {$ELSE}                                            { DPMI/WIN/NT/OS2 CODE }
   {$IFNDEF PPC_FPC}
   {$IFNDEF CPUI8086}HeapError := @HeapNotify;{$ENDIF}                          { Set heap error proc }
   {$ENDIF}
...
{$else fpc}
     Inc(Ofs(Cache^), SizeOf(TCache));          { Set cache offset }
{$endif fpc}
   End;
...
{$else fpc}
   Ofs(Cache^) := Ofs(P^)-SizeOf(TCache); { Previous cache }
   Seg(Cache^) := Seg(P^);                { Segment }
{$endif fpc}
...
{$else fpc}
       Dec(Ofs(P^),SizeOf(TBuffer));              { Correct to buffer }
{$endif fpc}
...
BEGIN
   ...
   Else Begin
     ...
     Buffer^.Master := @Seg(P^);                { Set master }
...
{$else fpc}
     Inc(Ofs(Buffer^), SizeOf(TBuffer));        { Buffer to data area }
{$endif fpc}
   End;
...
BEGIN
   ...
   If (BufHeapPtr = Seg(HeapEnd^)) Then Begin   { Is memory empty }
