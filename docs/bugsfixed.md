...
### BUG-041: ISCC.exe Wine AV in PopulateLanguageEntryData — KNOWN ISSUE (WINE)
**Found:** Jul 20 2026, ISCC_WINE_AV_BUG_REPORT.md
**Binary:** ISCC.exe + ISCmplr.dll (fpc264irc commit b7b5254f)
**Runtime:** Wine 9.0 + wine32:i386 on Ubuntu 24.04 x86_64
**Symptom:** Access violation (c0000005) at 0x0153DBCF inside ISCmplr.dll
after "Messages in script file" completes. Crash occurs in
PopulateLanguageEntryData during language codepage processing.
**Root cause:** Wine's GetCPInfoEx / MultiByteToWideChar implementation
is incomplete for certain codepages. PopulateLanguageEntryData calls
Win32 codepage APIs to build language conversion tables. Wine returns
garbage or zero for unsupported codepages, Inno dereferences → AV.
**What works under Wine:**
  - ISCC.exe launches, ISCmplr.dll loads
  - .iss script parsing (all sections)
  - PrepareSetupE32 succeeds (resources correct)
  - Default.isl message loading
  - ReadMessagesFromScript succeeds
  - Crash is AFTER message parsing, in codepage conversion
**What has been ruled out:**
  - Missing resources (all verified, RT_RCDATA #11111 present)
  - BUG-029 AnsiString fix (correct, unrelated)
  - BUG-032 fpcres LangID (fixed, unrelated)
  - PE resource directory layout (PrepareSetupE32 succeeds)
  - fpc264irc "inno wine fix" commit b7b5254f (does not fix this)
**Not an FPC bug. Not a code bug. Wine codepage limitation.**
**Fix:** Test on real Windows. Expected to work.
**Status:** KNOWN ISSUE — Wine only. Will not fix.

### BUG-042: Mystic Install Extraction Failure — FIXED
**Found:** Jul 25 2026
**Binary:** mystic.exe (fpc264irc Win32 build)
**Runtime:** Windows 11 (native, not Wine)
**Symptom:** `mystic -cfg` crashes with runtime error 3 (path not found)
on a fresh install. The setup wizard should prompt to create directories
but crashes before reaching the prompt.
**Call stack:** $00401F40 → $00402DEC → $0040DD81
**Suspected causes:**
  1. SysUtils init: GetModuleFileName/ExtractFilePath path corruption
  2. Windows 11 Terminal vs legacy conhost console API incompatibility
  3. RTL unit initialization trying to access non-existent path
**Test:** Try Compatibility Mode (Windows 7) or launch from cmd.exe
**Status:** OPEN — video evidence pending
**Note:** Running `mystic` after failed install shows same error (expected,
directories were never created). The root cause is in -cfg startup.

**UPDATE (Jul 25 2026):** Root cause found and fixed.
**Real cause:** `install_data.mys` was built with default FPC record
alignment (`{$PACKRECORDS DEFAULT}` = C-style, pads to even boundaries).
But `install.exe` reads it with `{$PACKRECORDS 1}` from `M_OPS.PAS`.

Record size mismatch:
  - `maHeaderRec`: 12 bytes (default) vs 11 bytes (packed)
  - `maFileHdrRec`: 100 bytes (default) vs 98 bytes (packed)

The installer opens the archive, reads the header at wrong offsets,
gets garbage file sizes, and silently fails to extract. No executables
are extracted. Running `mystic` afterward finds no `mystic.dat` →
runtime error 3.

**Fix:** Add `{$PACKRECORDS 1}` to `install_arc.pas` before the Type
block. Rebuild `install_data.mys` with the packed record layout.
Fixed `install_arc.pas` in `src/mystic_fixes/`.

**Verified:** 135/135 files extract correctly with the fix.

### BUG-042 UPDATE: Root Cause Found — PACKRECORDS Mismatch
**Root cause:** `install_data.mys` was built by `install_make` which 
compiles `install_arc.pas` WITHOUT `{$I M_OPS.PAS}`. This means 
install_make uses default record alignment ({$PACKRECORDS 4}), producing
12-byte main headers and 100-byte file records.

But `install.exe` includes `{$I M_OPS.PAS}` which sets `{$PACKRECORDS 1}`,
reading 11-byte headers and 98-byte file records.

Result: every field after the header is misaligned → garbage file sizes
→ extraction silently fails → no files extracted → mystic.exe missing
→ runtime error 3.

**Fix:** Add `{$PACKRECORDS 1}` to `install_arc.pas` itself (line 28),
so both install.exe and install_make use identical byte-packed records.
Then rebuild install_data.mys with the fixed install_make.

**Verified:** 135/135 files extract correctly after fix:
  - mystic.exe (717,824 bytes)
