...
{$ifdef cpu16}
function _MakeReplaceFlags(replAll, ignCase: Boolean): TReplaceFlags;
var b: Byte;
begin
  b := 0;
  if replAll then b := b or 1;
  if ignCase then b := b or 2;
  Move(b, _MakeReplaceFlags, 1);
end;
{$endif}
...
begin
  ...
  MatchesCount:=0;
  MatchesAllocatedLimit:=0;
  if (Byte(Flags) and 2 <> 0) then begin
    ...
    while MatchIndex <= MatchLimit do begin
      if (S[MatchIndex]=PatternLowerCase[OldPatternSize]) or (S[MatchIndex]=PatternUppercase[OldPatternSize]) then begin
        ...
        if MatchInternal=0 then begin
          ...
          if not ((Byte(Flags) and 1 <> 0)) then begin
            break;
          end;
        end else begin
          //Match not found
...
begin
  ...
  if (Byte(Flags) and 2 <> 0) then begin
    FindMatchesBoyerMooreCaseINSensitive(@s[1],@OldPattern[1],Length(S),Length(OldPattern),Matches, (Byte(Flags) and 1 <> 0));
  end else begin
    FindMatchesBoyerMooreCaseSensitive(@s[1],@OldPattern[1],Length(S),Length(OldPattern),Matches, (Byte(Flags) and 1 <> 0));
  end;
...
Function AnsiReplaceText(const AText, AFromText, AToText: string): string;inline;
begin
  Result := StringReplace(AText,AFromText,AToText,{$ifdef cpu16}_MakeReplaceFlags(true,true){$else}[rfReplaceAll,rfIgnoreCase]{$endif});
end;
...
Function AnsiReplaceStr(const AText, AFromText, AToText: string): string;inline;
begin
Result := StringReplace(AText,AFromText,AToText,{$ifdef cpu16}_MakeReplaceFlags(true,false){$else}[rfReplaceAll]{$endif});
end;
    ...
    begin
      ...
      begin
        ...
        if TextStr1 <> TextStr2 then
          {$ifdef cpu16}Result := AnsiCompareText(TextStr1, TextStr2){$else}Result := WideCompareText(UTF8Decode(TextStr1), UTF8Decode(TextStr2)){$endif}
        else
          Result := 0;
      end;
      if Result <> 0 then
...
begin
  ...
  while pc < lastpc do
    ...
    else if not ((Byte(Flags) and 1 <> 0)) then
      begin
