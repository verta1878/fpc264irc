...
import sys, os, time, re, getopt, codecs, warnings
 ...
 * obtained from %s.
 *
 * This file was generated by "%s %s" at
 * %s.
 * DO NOT EDIT!
 */
...
def write_oui_data (f):
  f.write (OUI_HEAD % (OUI_URL, sys.executable, __file__, time.ctime()))
...
def parse_oui_txt (lines):
  ...
  for line in lines:
      line = line.rstrip()
      if not re.match (r"^[0-9A-F]{2}-[0-9A-F]{2}-[0-9A-F]{2}[\t ]*\(hex\)[\t ]*", line):
         continue
      prefix = line [0:8]
...
def get_local_file_or_download (fname, url):
  ...
  #
  # And ignore this Python 3.14 warning:
