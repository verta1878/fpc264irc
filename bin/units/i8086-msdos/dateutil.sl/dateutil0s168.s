...
SECTION text
	ALIGN 2
	GLOBAL DATEUTILS_$$_INVALIDDATEWEEKERROR$WORD$WORD$WORD
DATEUTILS_$$_INVALIDDATEWEEKERROR$WORD$WORD$WORD:
		push	bp
		mov	bp,sp
		sub	sp,18
..@j2934:
		...
		mov	ax,1
		push	ax
		push	word [RESSTR_$SYSCONST_$$_SERRINVALIDDATEWEEK+2]
		mov	dx,word [bp+8]
		mov	ax,0
		mov	word [bp-16],dx
		mov	word [bp-14],ax
