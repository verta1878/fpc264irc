BITS 16
CPU 8086
SECTION text use16 class=code
SECTION rodata class=data
SECTION data class=data
SECTION fpc class=data
SECTION bss class=bss
SECTION stack stack class=stack align=16
SECTION heap class=heap align=16
GROUP dgroup rodata data fpc bss stack heap
SECTION text

SECTION text
	ALIGN 2
	GLOBAL MATRIX_$$_minus$TMATRIX4_SINGLE$$TMATRIX4_SINGLE
MATRIX_$$_minus$TMATRIX4_SINGLE$$TMATRIX4_SINGLE:
		push	bp
		mov	bp,sp
		mov	bx,word [bp+4]
		wait fld	dword [bx]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+4]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+4]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+8]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+8]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+12]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+12]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+16]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+16]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+20]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+20]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+24]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+24]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+28]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+28]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+32]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+32]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+36]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+36]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+40]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+40]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+44]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+44]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+48]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+48]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+52]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+52]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+56]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+56]
		DB	09bh
		mov	bx,word [bp+4]
		wait fld	dword [bx+60]
		wait fchs
		mov	bx,word [bp+6]
		wait fstp	dword [bx+60]
		DB	09bh
		mov	sp,bp
		pop	bp
		ret	4
