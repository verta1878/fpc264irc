{
  test_freebsd_sockets.pas — FreeBSD sockets verification test

  Proves:
    1. Uses Sockets compiles for FreeBSD (i386 and x86_64)
    2. Same Sockets API as Linux (both use rtl/unix/sockets.pp)
    3. Cross-compilation from Linux works with FreeBSD binutils

  Build (native on FreeBSD):
    ppc386 test_freebsd_sockets.pas
    or
    ppcx64 test_freebsd_sockets.pas

  Build (cross from Linux):
    ppc386 -Tfreebsd -Furtl/units/i386-freebsd test_freebsd_sockets.pas

  Run (on FreeBSD):
    ./test_freebsd_sockets
}
program test_freebsd_sockets;
...
begin
  ...
  WriteLn('FPC 2.6.4irc FreeBSD Sockets Test');
  WriteLn;
  ...
  Check('fpBind to port 19997 = 0', Err = 0);
